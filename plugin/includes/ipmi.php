...
function get_bmc_status() {
    ...
    if (!$result['success']) {
        return [
            'connected' => false,
            'power' => false,
            'message' => is_array($result['output']) ? implode("\n", $result['output']) : 'BMC connection failed'
        ];
    }
    
    $output = is_array($result['output']) ? implode("\n", $result['output']) : '';
...
function execute_power_action($action) {
    ...
    if ($result['success']) {
        ...
        return handle_error("Failed to execute power action '$action': " . (is_array($result['output']) ? implode("\n", $result['output']) : 'Unknown error'));
    }
}
